...
public class AnalyzeAction extends Action {
	...
	public void actionPerformed(ActionEvent e) {
                ...
                {               
                  ...
                  if(dalsiP.getIsIterationStart()){
                      ...
                      for(Arc arc: dalsiP.getConnectedArcsNext()){
                          if(arc.getIsIterationArc()){
                              if(arc.getZmena()){lam = lam*(1 - arc.getChangedProbability());}
                              else{lam  = lam*(1 - arc.getProbability());}
                          }
                      }
                  }
                  ...
                  if(dalsiT.getMM1()){MM1(dalsiT, lam);dalsiT.SetProbability(caseprob);casetime += dalsiT.getSystemTime()*caseprob;casewaittime += dalsiT.getWaitingTime()*caseprob;}
                  if(dalsiT.getMMc()){MMc(dalsiT, lam);dalsiT.SetProbability(caseprob);casetime += dalsiT.getSystemTime()*caseprob;casewaittime += dalsiT.getWaitingTime()*caseprob;}
        ...
        private Place OR(Place zaciatok, double inputprob){
            ...
            while(zaciatok.getORJoin() != dalsiP){
                      if(zacinamOR == true){
                          dalsiT = arc.getTransition();zacinamOR=false;
                          if(dalsiT.getMM1()){MM1(dalsiT, lam*probarc);dalsiT.SetProbability(probarc);casetime += dalsiT.getSystemTime()*probarc;casewaittime += dalsiT.getWaitingTime()*probarc;}
                          if(dalsiT.getMMc()){MMc(dalsiT, lam*probarc);dalsiT.SetProbability(probarc);casetime += dalsiT.getSystemTime()*probarc;casewaittime += dalsiT.getWaitingTime()*probarc;}
        ...
        private Place OR(Place zaciatok, double inputprob, Arc timearc){
            ...
            while(zaciatok.getORJoin() != dalsiP){
                      if(zacinamOR == true){
                          ...
                          for(Arc arct : petriNet.getArcs()){
                            if (arct.equals(timearc)){
                                if(dalsiT.getMM1()){MM1(dalsiT, lam*probarc);dalsiT.SetProbability(probarc);arct.setArcTime((arct.getArcTime() + dalsiT.getSystemTime()*probarc));arct.setArcWaitTime((arc.getArcWaitTime() + dalsiT.getWaitingTime()*probarc));}
                                if(dalsiT.getMMc()){MMc(dalsiT, lam*probarc);dalsiT.SetProbability(probarc);arct.setArcTime((arct.getArcTime() + dalsiT.getSystemTime()*probarc));arct.setArcWaitTime((arc.getArcWaitTime() + dalsiT.getWaitingTime()*probarc));}
                    ...
                    for(Arc arcc: dalsiP.getConnectedArcsNext()){
                      ...
                      for(Arc arct : petriNet.getArcs()){
                            if (arct.equals(timearc)){
                                if(dalsiT.getMM1()){MM1(dalsiT, lam*probarc);dalsiT.SetProbability(probarc);arct.setArcTime((arc.getArcTime() + dalsiT.getSystemTime()*probarc));arct.setArcWaitTime((arc.getArcWaitTime() + dalsiT.getWaitingTime()*probarc));}
                                if(dalsiT.getMMc()){MMc(dalsiT, lam*probarc);dalsiT.SetProbability(probarc);arct.setArcTime((arc.getArcTime() + dalsiT.getSystemTime()*probarc));arct.setArcWaitTime((arc.getArcWaitTime() + dalsiT.getWaitingTime()*probarc));}
                            }
                          }
                      }
        ...
        private Transition AND(Transition zaciatok, double inputprob){
            ...
            while(zaciatok.getANDJoin() != dalsiT){
                      if(zacinamAND == true){dalsiP = arc.getPlace();zacinamAND=false;}
                      else{
                      dalsiP = dalsiT.getConnectedArcNext().getPlace();
                      }
                      System.out.println(dalsiP.getLabel().getText());
                    ...
                    for(Arc arcc: dalsiP.getConnectedArcsNext()){
                      ...
                      if(dalsiT.getMM1()){MM1(dalsiT, lam*inputprob);dalsiT.SetProbability(inputprob);arc.setArcTime((arc.getArcTime() + dalsiT.getSystemTime()*inputprob));arc.setArcWaitTime((arc.getArcWaitTime() + dalsiT.getWaitingTime()*inputprob));}
                      if(dalsiT.getMMc()){MMc(dalsiT, lam*inputprob);dalsiT.SetProbability(inputprob);arc.setArcTime((arc.getArcTime() + dalsiT.getSystemTime()*inputprob));arc.setArcWaitTime((arc.getArcWaitTime() + dalsiT.getWaitingTime()*inputprob));}
        ...
        private Place ITER(Place zaciatok, double inputprob){
            ...
            while(zaciatok.getIterationEnd() != dalsiP){
                      if(zacinamITER == true){
                          dalsiT = zaciatok.getConnectedArcNext().getTransition();zacinamITER=false;
                          if(dalsiT.getMM1()){MM1(dalsiT, lam*probarcup);dalsiT.SetProbability(probarcup);casetime += dalsiT.getSystemTime()*probarcup;casewaittime += dalsiT.getWaitingTime()*probarcup;}
                          if(dalsiT.getMMc()){MMc(dalsiT, lam*probarcup);dalsiT.SetProbability(probarcup);casetime += dalsiT.getSystemTime()*probarcup;casewaittime += dalsiT.getWaitingTime()*probarcup;}
            ...
            System.out.println("Skoncil som hronu iter vetvu");
             ...
             while(dalsiP != zaciatok){
                 ...
                 if(zacinamITERdown == true){
                          for(Arc arc : zaciatok.getIterationEnd().getConnectedArcsNext()){
                                if (arc.getIsIterationArc()){
                                    dalsiT = arc.getTransition();zacinamITERdown=false;
                                 }
                          }
                          if(dalsiT.getMM1()){MM1(dalsiT, lam*probarcdown);dalsiT.SetProbability(probarcdown);casetime += dalsiT.getSystemTime()*probarcdown;casewaittime += dalsiT.getWaitingTime()*probarcdown;}
                          if(dalsiT.getMMc()){MMc(dalsiT, lam*probarcdown);dalsiT.SetProbability(probarcdown);casetime += dalsiT.getSystemTime()*probarcdown;casewaittime += dalsiT.getWaitingTime()*probarcdown;}
            ...
            while(zaciatok.getIterationEnd() != dalsiP){
                      if(zacinamITER == true){
                          dalsiT = zaciatok.getConnectedArcNext().getTransition();zacinamITER=false;
                           for(Arc arct : petriNet.getArcs()){
                            if (arct.equals(timearc)){
                          if(dalsiT.getMM1()){MM1(dalsiT, lam*probarcup);dalsiT.SetProbability(probarcup);arct.setArcTime((arct.getArcTime() + dalsiT.getSystemTime()*probarcup));arct.setArcWaitTime((arct.getArcWaitTime() + dalsiT.getWaitingTime()*probarcup));}
                          if(dalsiT.getMMc()){MMc(dalsiT, lam*probarcup);dalsiT.SetProbability(probarcup);arct.setArcWaitTime((arct.getArcWaitTime() + dalsiT.getSystemTime()*probarcup));arct.setArcWaitTime((arct.getArcWaitTime() + dalsiT.getWaitingTime()*probarcup));}
             ...
             while(zaciatok != dalsiP){
                 if(zacinamITERdown == true){
                          ...
                          for(Arc arct : petriNet.getArcs()){
                            if (arct.equals(timearc)){
                          if(dalsiT.getMM1()){MM1(dalsiT, lam*probarcdown);dalsiT.SetProbability(probarcdown);arct.setArcTime((arct.getArcTime() + dalsiT.getSystemTime()*probarcdown));arct.setArcWaitTime((arct.getArcWaitTime() + dalsiT.getWaitingTime()*probarcdown));}
                          if(dalsiT.getMMc()){MMc(dalsiT, lam*probarcdown);dalsiT.SetProbability(probarcdown);arct.setArcWaitTime((arct.getArcWaitTime() + dalsiT.getSystemTime()*probarcdown));arct.setArcWaitTime((arct.getArcWaitTime() + dalsiT.getWaitingTime()*probarcdown));}
        ...
        public double maxwtime(Transition tran){
            ...
            for(Arc arc : tran.getConnectedArcsNext()){
                pom = arc.getArcTime();
                if (pom > maxtime){maxtime=pom; maxwtime=arc.getArcWaitTime();}
            }
            return maxwtime;
        }
