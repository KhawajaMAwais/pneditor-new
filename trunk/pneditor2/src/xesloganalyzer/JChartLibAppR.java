...
public class JChartLibAppR extends JFrame implements WindowListener {
 ...
 */
public JChartLibAppR(ArrayList<XESResource> resources,String chattittle,String xtitle,String ytitle) {
...
//    values1[4] = 2;
    ...
    for(XESResource resource: resources){
        JChartLibSerie serie = new JChartLibSerie(resource.getName());
        serie.addValue(resource.getCount());
...
private JChartLibBaseChart createChart(final JChartLibDataSet dataset) {
    ...
    try {
        chart.saveAsJPEG("grafR.jpg", 600, 400);
    } catch (IOException ex) {
        Logger.getLogger(JChartLibAppR.class.getName()).log(Level.SEVERE, null, ex);
    }
    return chart;
}
